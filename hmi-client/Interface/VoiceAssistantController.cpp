...
{
    m_messages.appendMessage(
        QStringLiteral("你好，我是 DrivePilot 车载 AI Agent。你可以键盘输入，也可以使用讯飞语音输入。"
                       "任务会由 Python FastAPI 后端调用 Kimi 进行分析和多步工具编排，"
                       "界面会显示分析摘要、执行计划、真实车机操作结果与最终回复。"),
...
void VoiceAssistantController::loadApiCredentials()
{
    ...
    for (const QString &candidate : candidates) {
        ...
        if (looksLikeCredentialPlaceholder(appId)
            ...
            lastError = QStringLiteral("%1 中仍是占位内容，请填写真实讯飞凭据")
...
void VoiceAssistantController::handleAgentSocketMessage(const QString &message)
{
    ...
    if (type == QStringLiteral("connected")) {
        ...
        setAgentStatus(object.value(QStringLiteral("model_configured")).toBool()
                           ? QStringLiteral("Kimi Agent 在线")
                           : QStringLiteral("本地演示 Agent 在线"));
        return;
    }
    ...
    if (type == QStringLiteral("analysis")) {
        ...
        if (!content.isEmpty())
            m_messages.appendMessage(content, QStringLiteral("agent"), QStringLiteral("分析摘要"));
        setAgentStatus(QStringLiteral("正在思考"));
    ...
    if (type == QStringLiteral("plan")) {
        const QString content = object.value(QStringLiteral("content")).toString().trimmed();
        if (!content.isEmpty())
            m_messages.appendMessage(content, QStringLiteral("agent"), QStringLiteral("执行计划"));
        setAgentStatus(QStringLiteral("准备执行工具"));
        setCurrentTaskText(content);
        return;
    }
    ...
    if (type == QStringLiteral("final")) {
        const QString content = object.value(QStringLiteral("content")).toString().trimmed();
        if (!content.isEmpty())
            m_messages.appendMessage(content, QStringLiteral("assistant"), QStringLiteral("最终回复"));
