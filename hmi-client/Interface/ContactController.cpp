...
void ContactController::seedContacts()
{
    ...
    const QList<QPair<QString, QString>> defaults = {
        ...
        {QStringLiteral("默认联系人"), QStringLiteral("13800000000")}
    };
...
void ContactController::importConfiguredContact()
{
    for (const QString &path : configCandidates()) {
        ...
        const QString phone = normalizePhone(
            object.value(QStringLiteral("default_phone")).toString());
        if (phone.size() < 3)
            return;

        QString name = object.value(QStringLiteral("default_contact_name")).toString().trimmed();
        if (name.isEmpty())
        ...
        query.addBindValue(now);
        if (!query.exec())
            qWarning() << "Import default contact failed:" << query.lastError();
        return;
    }
}
...
bool ContactController::dialPhoneNumber(const QString &phone, const QString &displayName)
{
    ...
    if (accepted) {
        ...
        emit noticeRequested(QStringLiteral(
            "无法直接拨号：当前 Linux 系统未配置 tel: 协议处理程序"));
#else
        emit noticeRequested(QStringLiteral(
            "无法直接拨号：当前系统未配置 tel: 协议处理程序"));
#endif
    }
    return accepted;
}
