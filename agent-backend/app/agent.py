...
from .demo_agent import build_demo_plan
...
class AgentRunner:
        ...
        await send_event(
            {
                "type": "analysis",
                "phase": "understanding",
                "content": "正在结合用户指令与可用车机工具分析任务。",
            }
        )

        try:
            if self.kimi_client is None:
                await self._run_demo(session, user_text, send_event)
            else:
                ...
                await send_event(
                    {
                        "type": "analysis",
                        "phase": "fallback",
                        "content": f"云端模型调用失败，切换本地演示计划：{exc}",
                    }
                )
                await self._run_demo(session, user_text, send_event)
            else:
    ...
    async def _run_kimi(self, session: AgentSession, send_event: SendEvent) -> None:
        ...
        for step in range(1, self.settings.max_agent_steps + 1):
                ...
                await send_event(
                    {
                        "type": "analysis",
                        "phase": "thinking",
                        "content": f"第 {step} 轮：模型正在推理并选择下一步动作。",
                    }
                )
            ...
            await send_event(
                {
                    "type": "plan",
                    "content": "计划执行：" + " → ".join(tool_title(call.name) for call in turn.tool_calls),
            ...
            await send_event(
                {
                    "type": "analysis",
                    "phase": "observation",
                    "content": "已获得车机执行结果，正在判断是否需要继续操作。",
                }
            )
    ...
    async def _run_demo(
        ...
        plan = build_demo_plan(user_text)
        await send_event(
            {
                "type": "analysis",
                "phase": "demo",
        ...
        await send_event(
            {
                "type": "plan",
                "content": "本地计划：" + " → ".join(tool_title(call.name) for call in plan.tool_calls),
