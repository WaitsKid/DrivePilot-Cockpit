...
from dms_backend.evaluation.thresholds import (
    PredictionRecord,
    _predictions_from_risk_threshold,
    find_best_risk_threshold,
)
...
def _load_evaluation_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise ValueError(f"evaluation 配置不是有效对象: {path}")
    return raw
...
def evaluate_task(
    *,
    ...
    config = _load_evaluation_config(evaluation_config_path)
    task_config = config["tasks"].get(task_name)
    if not isinstance(task_config, dict):
        raise ValueError(f"evaluation 配置中不存在任务: {task_name}")
