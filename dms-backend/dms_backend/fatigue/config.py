...
def load_service_config(path: str | Path | None = None) -> DmsServiceConfig:
    config_path = Path(path) if path is not None else project_root() / "configs" / "service.yaml"
    if not config_path.is_absolute():
        config_path = project_root() / config_path
    if not config_path.is_file():
        raise FileNotFoundError(f"找不到 DMS 服务配置文件: {config_path}")

    with config_path.open("r", encoding="utf-8") as file:
        raw = yaml.safe_load(file)
    if not isinstance(raw, dict):
        raise ValueError(f"DMS 服务配置不是有效对象: {config_path}")
    ...
    inference_config_value = raw.get("inference_config", "configs/inference.yaml")
    return DmsServiceConfig(
        path=config_path.resolve(),
        inference_config_path=resolve_project_path(inference_config_value).resolve(),
