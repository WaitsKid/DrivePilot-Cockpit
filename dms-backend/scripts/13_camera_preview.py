...
def main() -> None:
    ...
    output_dir = resolve_project_path(config.get("output_root", "artifacts/inference")) / "camera"
    output_dir.mkdir(parents=True, exist_ok=True)
    frame_index = 0
    latest_result = None
    timestamps: deque[float] = deque(maxlen=30)

    print("Camera preview started. Press Q/Esc to quit, S to save the current frame.")
    try:
        while True:
            ...
            cv2.imshow("DriveGuard DMS inference", preview)
