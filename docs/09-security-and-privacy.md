...
| 服务 | 风险 | 控制 |
|---|---|---|
| Kimi | 用户文本发送云端 | 文档告知；不上传摄像头；后续可脱敏 |
...
| 威胁 | 当前措施 | 生产化改进 |
|---|---|---|
| 本地端口被其他程序访问 | 绑定 `127.0.0.1` | 进程认证、随机令牌、Unix socket |
| WebSocket 消息伪造 | 工具白名单和参数校验 | 双向认证、签名、权限域 |
| 模型提示注入 | 工具白名单 | 策略引擎、用户确认、审计 |
| Qt 反编译 | 云模型 Key 不在 Qt | 设备身份和后端网关 |
| 摄像头滥用 | 开关可释放摄像头 | OS 权限、明显指示、审计 |
| 依赖供应链 | requirements 范围约束 | 锁文件、SCA、镜像签名 |
