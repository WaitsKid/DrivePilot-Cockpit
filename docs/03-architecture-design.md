...
| `agent` | ReAct 式“模型判断—工具调用—Observation—继续”循环 |
...
### 决策 4：Agent 使用 ReAct 式闭环

Agent 不预先生成不可变的完整计划。每一轮模型根据会话与上一轮工具返回结果决定下一步：继续调用工具、修正参数，或者生成最终回复。Qt 返回的 `tool_result` 作为 Observation 重新加入模型上下文，服务端使用 `call_id` 关联调用和结果，并限制单次工具超时与最大执行轮数。

### 决策 5：导航轨迹在 C++ 中维护

高德 JS API 负责地点搜索、路线规划和地图渲染，C++ `MapController` 负责解析路线步骤、累计轨迹距离、车辆位置插值、车头方向和导航进度。QML 捕获 W/S 键并按时间步长推进进度，再通过 QWebChannel 将车辆状态同步到网页地图。这样地图视图和导航状态分离，也避免每次按键都重新请求路线。
